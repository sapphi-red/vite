...
async function compileCSS(
  ...
  const postcssMap = formatPostcssSourceMap(postcssResult.map.toJSON(), cleanId)
  const combinedMap = preprocessorMap
    ? combineSourcemaps(cleanId, [
...
function formatPostcssSourceMap(
  ...
  const sources = rawMap.sources
    ...
    .map((source) => {
      const sourceWithoutQuery = cleanUrl(decodeURIComponent(source))
      return normalizePath(path.resolve(inputFileDir, sourceWithoutQuery))
    })
