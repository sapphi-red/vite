  ...
  return {
    ...
    options(options) {
      if (this.meta.watchMode) {
        ...
        options.external = options.external.concat(pkg.devDependencies)
      }
    },
  }
}
  ...
  return {
    ...
    transform: {
      filter: {
        id: new RegExp(
          `(?:${Object.keys(deps)
            // TODO: remove when https://github.com/rolldown/rolldown/pull/3955 is released
            .flatMap((f) => [f, f.replaceAll('/', '\\\\')])
            .join('|')})$`,
        ),
      },
      handler(code, id) {
